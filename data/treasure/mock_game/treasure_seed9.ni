...
The r_10 and the r_11 and the r_12 and the r_14 and the r_13 and the r_15 and the r_16 and the r_17 and the r_9 and the r_2 and the r_1 and the r_3 and the r_0 and the r_4 and the r_5 and the r_6 and the r_7 and the r_8 and the r_18 and the r_19 are rooms.
...
The basement part 1 is some text that varies. The basement part 1 is "[if c_0 is open and the c_0 contains nothing] The cabinet is empty! What a waste of a day![end if]".
The basement part 2 is some text that varies. The basement part 2 is " You make out a workbench. The workbench is typical.[if there is something on the s_0] On the workbench you make out [a list of things on the s_0].[end if]".
The basement part 3 is some text that varies. The basement part 3 is "[if there is nothing on the s_0] However, the workbench, like an empty workbench, has nothing on it.[end if]".
...
The sauna part 1 is some text that varies. The sauna part 1 is " normal looking safe in the room.[if c_1 is open and there is something in the c_1] The safe contains [a list of things in the c_1]. You idly wonder how they came up with the name TextWorld for this place. It's pretty fitting.[end if]".
...
The spare room part 1 is some text that varies. The spare room part 1 is " coffer.[if c_2 is open and there is something in the c_2] The coffer contains [a list of things in the c_2]. You check the price tag that's still affixed to the coffer. Eighty dollars?! That's ridiculous! You know, I know a person who could get you one of those for 30 bucks![end if]".
The spare room part 2 is some text that varies. The spare room part 2 is "[if c_2 is open and the c_2 contains nothing] The coffer is empty! This is the worst thing that could possibly happen, ever![end if]".
...
The dish-pit part 0 is some text that varies. The dish-pit part 0 is "Well, here we are in a dish-pit. You decide to just list off a complete list of everything you see in the room, because hey, why not?
...
The garage part 1 is some text that varies. The garage part 1 is " locker here.[if c_3 is open and there is something in the c_3] The locker contains [a list of things in the c_3].[end if]".
...
The garage part 3 is some text that varies. The garage part 3 is " Look over there! a counter. [if there is something on the s_1]On the counter you make out [a list of things on the s_1].[end if]".
The garage part 4 is some text that varies. The garage part 4 is "[if there is nothing on the s_1]Unfortunately, there isn't a thing on it. What, you think everything in TextWorld should have stuff on it?[end if]".
...
The attic part 0 is some text that varies. The attic part 0 is "Okay, so you're in an attic, cool, but is it usual? You better believe it is.
...
The attic part 2 is some text that varies. The attic part 2 is " You make out a rack. [if there is something on the s_2]You see [a list of things on the s_2] on the rack.[end if]".
The attic part 3 is some text that varies. The attic part 3 is "[if there is nothing on the s_2]Looks like someone's already been here and taken everything off it, though.[end if]".
The attic part 4 is some text that varies. The attic part 4 is "

There is an exit to the east. Don't worry, it is unguarded. There is an unguarded exit to the north. There is an unguarded exit to the south. You need an unguarded exit? You should try going west.".
...
There is an exit to the east. Don't worry, it is unguarded. You need an unguarded exit? You should try going north. You don't like doors? Why not try going south, that entranceway is unblocked. There is an exit to the west. Don't worry, it is unblocked.".
...
The cubicle part 1 is some text that varies. The cubicle part 1 is " display.[if c_5 is open and there is something in the c_5] The display contains [a list of things in the c_5]. Is this it? Is this what you came to TextWorld to see? a display?[end if]".
The cubicle part 2 is some text that varies. The cubicle part 2 is "[if c_5 is open and the c_5 contains nothing] What a letdown! The display is empty![end if]".
The cubicle part 3 is some text that varies. The cubicle part 3 is " Hey, want to see a bureau? Look over there, a bureau.[if c_6 is open and there is something in the c_6] The bureau contains [a list of things in the c_6]. You idly wonder how they came up with the name TextWorld for this place. It's pretty fitting.[end if]".
The cubicle part 4 is some text that varies. The cubicle part 4 is "[if c_6 is open and the c_6 contains nothing] Empty! What kind of nightmare TextWorld is this?[end if]".
...
The bedroom part 1 is some text that varies. The bedroom part 1 is " basket in the room.[if c_7 is open and there is something in the c_7] The basket contains [a list of things in the c_7].[end if]".
...
The vault part 0 is some text that varies. The vault part 0 is "You are in a vault. An ordinary kind of place. The room is well lit.
 ...
 You make out a shelf. [if there is something on the s_3]On the shelf you can make out [a list of things on the s_3]. You shudder, but continue examining the room.[end if]".
The vault part 1 is some text that varies. The vault part 1 is "[if there is nothing on the s_3]Unfortunately, there isn't a thing on it. Hopefully, this discovery doesn't ruin your TextWorld experience![end if]".
The vault part 2 is some text that varies. The vault part 2 is " You can make out a table. [if there is something on the s_4]On the table you make out [a list of things on the s_4]. Wow! Just like in the movies![end if]".
The vault part 3 is some text that varies. The vault part 3 is "[if there is nothing on the s_4]But the thing hasn't got anything on it. What, you think everything in TextWorld should have stuff on it?[end if]".
...
The bathroom part 0 is some text that varies. The bathroom part 0 is "You've just walked into a bathroom.
 ...
 You make out a trunk. Wow, isn't TextWorld just the best?[if c_8 is open and there is something in the c_8] The trunk contains [a list of things in the c_8].[end if]".
The bathroom part 1 is some text that varies. The bathroom part 1 is "[if c_8 is open and the c_8 contains nothing] The trunk is empty! What a waste of a day![end if]".
The bathroom part 2 is some text that varies. The bathroom part 2 is " You see [if c_9 is locked]a locked[else if c_9 is open]an opened[otherwise]a closed[end if]".
The bathroom part 3 is some text that varies. The bathroom part 3 is " dresser close by.[if c_9 is open and there is something in the c_9] The dresser contains [a list of things in the c_9].[end if]".
The bathroom part 4 is some text that varies. The bathroom part 4 is "[if c_9 is open and the c_9 contains nothing] Empty! What kind of nightmare TextWorld is this?[end if]".
...
The lounge part 0 is some text that varies. The lounge part 0 is "You've entered a lounge. You begin looking for stuff.
 ...
 You can see a desk. You shudder, but continue examining the desk. [if there is something on the s_5]You see [a list of things on the s_5] on the desk.[end if]".
The lounge part 1 is some text that varies. The lounge part 1 is "[if there is nothing on the s_5]The desk appears to be empty. You make a mental note to not get your hopes up the next time you see a desk in a room.[end if]".
...
The study part 0 is some text that varies. The study part 0 is "You've just sauntered into a study.
 ...
 As if things weren't amazing enough already, you can even see a case.[if c_10 is open and there is something in the c_10] The case contains [a list of things in the c_10].[end if]".
The study part 1 is some text that varies. The study part 1 is "[if c_10 is open and the c_10 contains nothing] The case is empty, what a horrible day![end if]".
The study part 2 is some text that varies. The study part 2 is " [if c_11 is locked]A locked[else if c_11 is open]An open[otherwise]A closed[end if]".
The study part 3 is some text that varies. The study part 3 is " box, which looks ordinary, is in the corner.[if c_11 is open and there is something in the c_11] The box contains [a list of things in the c_11].[end if]".
The study part 4 is some text that varies. The study part 4 is "[if c_11 is open and the c_11 contains nothing] The box is empty, what a horrible day![end if]".
...
The kitchen part 0 is some text that varies. The kitchen part 0 is "You have walked into a kitchen. Not the kitchen you'd expect. No, this is a kitchen.
 ...
 You can make out a freezer.[if c_12 is open and there is something in the c_12] The freezer contains [a list of things in the c_12].[end if]".
The kitchen part 1 is some text that varies. The kitchen part 1 is "[if c_12 is open and the c_12 contains nothing] Empty! What kind of nightmare TextWorld is this?[end if]".
The kitchen part 2 is some text that varies. The kitchen part 2 is " You make out a refrigerator.[if c_13 is open and there is something in the c_13] The refrigerator contains [a list of things in the c_13].[end if]".
The kitchen part 3 is some text that varies. The kitchen part 3 is "[if c_13 is open and the c_13 contains nothing] The refrigerator is empty, what a horrible day![end if]".
...
The r_10 and the r_11 and the r_12 and the r_14 and the r_13 and the r_15 and the r_16 and the r_17 and the r_9 and the r_2 and the r_1 and the r_3 and the r_0 and the r_4 and the r_5 and the r_6 and the r_7 and the r_8 and the r_18 and the r_19 are rooms.
The r_10 and the r_11 and the r_12 and the r_14 and the r_13 and the r_15 and the r_16 and the r_17 and the r_9 and the r_2 and the r_1 and the r_3 and the r_0 and the r_4 and the r_5 and the r_6 and the r_7 and the r_8 and the r_18 and the r_19 are privately-named.
...
The c_11 is in r_18.
The c_11 is closed.
The description of c_12 is "The freezer looks strong, and impossible to break. [if open]You can see inside it.[else if closed]You can't see inside it because the lid's in your way.[otherwise]There is a lock on it.[end if]".
The printed name of c_12 is "freezer".
Understand "freezer" as c_12.
The c_12 is in r_19.
The c_12 is locked.
The description of c_13 is "The refrigerator looks strong, and impossible to break. [if open]It is open.[else if closed]It is closed.[otherwise]It is locked.[end if]".
The printed name of c_13 is "refrigerator".
Understand "refrigerator" as c_13.
The c_13 is in r_19.
The c_13 is closed.
...
Test quest0_0 with "go south / go east / go south / go west / go south / go west / go west / go north / go east / go north / go north / go west / go west / take paper towel from safe"
...
Every turn:
	...
	else if The player is in r_12 and The c_1 is in r_12 and The c_1 is open and The player carries the o_0:
...
The objective part 0 is some text that varies. The objective part 0 is "Welcome to TextWorld! First of all, make an attempt to head south. That done, go to the east. After that, venture south. With that over with, move west. If you can get around to doing that, head south".
The objective part 1 is some text that varies. The objective part 1 is ". Okay, and then, make an effort to move west. And then, travel west. After that, head north. Once you do that, try to take a trip east. That done, go to the north. With that over with, head north. An".
The objective part 2 is some text that varies. The objective part 2 is "d then, take a trip west. Okay, and then, make an attempt to travel west. Then, pick up the paper towel from the safe in the sauna. Alright, thanks!".
